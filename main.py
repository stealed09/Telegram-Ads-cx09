...
async def cmd_start(message: Message) -> None:
    ...
    if uid == config.ADMIN_ID:
        ...
        b = InlineKeyboardBuilder()
        b.button(text="🏠 Main Menu",    callback_data="main_menu")
        b.button(text="👑 Admin Panel",  callback_data="admin_menu")
        b.adjust(2)
...
async def cb_approve(call: CallbackQuery) -> None:
    ...
    b = InlineKeyboardBuilder()
    b.button(text="🏠 Open Menu", callback_data="main_menu")
    await bot.send_message(uid, config.MSG_APPROVED, reply_markup=b.as_markup())
...
async def cb_scraper_menu(call: CallbackQuery, state: FSMContext) -> None:
    ...
    b = InlineKeyboardBuilder()
    if folders:
        b.button(text="🔢 Enter Group ID manually", callback_data="scrape_manual")
        for fname, grps in folders.items():
            for g in grps[:10]:  # max 10 per folder shown
                b.button(
                    text=f"📂 {g['title'][:28]}",
                    callback_data=f"scrape_gid:{g['id']}",
                )
    else:
        b.button(text="🔢 Enter Group ID manually", callback_data="scrape_manual")
    b.button(text="🔙 Main Menu", callback_data="main_menu")
    b.adjust(1)
    await call.message.edit_text(
        "🔍 <b>Member Scraper</b>\n\nSelect a group or enter ID manually:",
        reply_markup=b.as_markup(),
    )

@router.callback_query(F.data == "scrape_manual")
@approved_only
async def cb_scrape_manual(call: CallbackQuery, state: FSMContext) -> None:
...
@router.callback_query(F.data.startswith("scrape_gid:"))
@approved_only
async def cb_scrape_gid(call: CallbackQuery, state: FSMContext) -> None:
    ...
    b.adjust(2)
    await call.message.edit_text("Choose export format:", reply_markup=b.as_markup())
...
async def cmd_accounts(message: Message) -> None:
    ...
    if not accounts:
        ...
        b.button(text="🔙 Main Menu",   callback_data="main_menu")
        b.adjust(1)
        await message.answer("No accounts linked yet.", reply_markup=b.as_markup())
        return
    b = InlineKeyboardBuilder()
    for acc in accounts:
        me   = acc["me"]
...
async def cmd_help(message: Message) -> None:
    ...
    await message.answer(
        ...
        "<b>Features available from menu:</b>\n"
        ...
        "• 👻 Ghost mode\n"
        "• 🔐 Manage accounts\n\n"
        "<i>Use the menu button below to get started.</i>",
        reply_markup=b.as_markup(),
    )
...
async def on_startup(bot: Bot) -> None:
    ...
    await bot.set_my_commands([
        BotCommand(command="start", description="Start / Request access"),
